...
function Hero(engine, sheet) {
    this.engine = engine;
    this.sheet = new Spritesheet(this.engine, sheet, 3, 8, 60);
...
Hero.prototype = {
    registerEvents: function() {
        ...
        this.engine.addKeyboardEvent(Keyboard.RIGHT, function() {
            if (hero.x + 50 >= engine.canvasHelper.width())
                return;
        ...
        this.engine.addKeyboardEvent(Keyboard.DOWN, function() {
            if (hero.y + 50 >= engine.canvasHelper.height())
                return;
        ...
        this.engine.addKeyboardReleaseEvent(function() {
            if (hero.directionX == Hero.LEFT || hero.directionY == Hero.UP)
                hero.sheet.column = 1;
            else if (hero.directionX == Hero.RIGHT || hero.directionY == Hero.DOWN)
                hero.sheet.column = 0;
